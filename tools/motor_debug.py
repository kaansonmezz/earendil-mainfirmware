...
class EarendilControlGui(QMainWindow):
    ...
    def __init__(self):
        ...
        # -- Debug Motor Test state (hold O to run) ----------------------
        #   Per-motor: direction "Stop"/"Forward"/"Backward"/"Brake",
        #   control "RPM"/"Duty", value int.
        self._motor_test_dir: dict[str, str] = {
            "FL": "Stop", "FR": "Stop", "RL": "Stop", "RR": "Stop"
        }
        self._motor_test_ctrl: dict[str, str] = {
            "FL": "RPM", "FR": "RPM", "RL": "RPM", "RR": "RPM"
        }
        self._motor_test_value: dict[str, int] = {
            "FL": 0, "FR": 0, "RL": 0, "RR": 0
        }
        self._motor_test_o_held: bool = False
        ...
        left_layout.addWidget(self._build_motor_test_group())
    ...
    # -- Debug Motor Test Panel (Hold O to run) ----------------------------

    _MOTOR_TEST_MOTORS = ("FL", "FR", "RL", "RR")
    _MOTOR_TEST_DIRS = ("Stop", "Forward", "Backward", "Brake")
    _MOTOR_TEST_CTRLS = ("RPM", "Duty")

    def _build_motor_test_group(self) -> QGroupBox:
        """Debug-only per-motor test panel.  Hold O to send commands."""
        grp = QGroupBox("Motor Test (Hold O)")
        grp.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Fixed)
        lay = QVBoxLayout(grp)
        lay.setContentsMargins(6, 4, 6, 4)
        lay.setSpacing(4)
        ...
        hdr = QHBoxLayout()
        hdr.setSpacing(4)
        hdr.addWidget(self._mt_label("", 48))
        hdr.addWidget(self._mt_label("Direction", 100))
        hdr.addWidget(self._mt_label("Ctrl", 50))
        hdr.addWidget(self._mt_label("Value", 60))
        lay.addLayout(hdr)

        self._mt_dir_combos: dict[str, QComboBox] = {}
        self._mt_ctrl_combos: dict[str, QComboBox] = {}
        self._mt_value_edits: dict[str, QLineEdit] = {}

        for motor in self._MOTOR_TEST_MOTORS:
            row = QHBoxLayout()
            row.setSpacing(4)
            row.addWidget(self._mt_label(motor, 48))

            dir_combo = QComboBox()
            dir_combo.addItems(self._MOTOR_TEST_DIRS)
            dir_combo.setFixedWidth(100)
            dir_combo.currentTextChanged.connect(
                lambda val, m=motor: self._on_mt_dir_changed(m, val))
            row.addWidget(dir_combo)
            self._mt_dir_combos[motor] = dir_combo

            ctrl_combo = QComboBox()
            ctrl_combo.addItems(self._MOTOR_TEST_CTRLS)
            ctrl_combo.setFixedWidth(50)
            ctrl_combo.currentTextChanged.connect(
                lambda val, m=motor: self._on_mt_ctrl_changed(m, val))
            row.addWidget(ctrl_combo)
            self._mt_ctrl_combos[motor] = ctrl_combo

            value_edit = QLineEdit("0")
            value_edit.setFixedWidth(60)
            value_edit.setAlignment(Qt.AlignCenter)
            value_edit.setPlaceholderText("0")
            value_edit.textChanged.connect(
                lambda val, m=motor: self._on_mt_value_changed(m, val))
            row.addWidget(value_edit)
            self._mt_value_edits[motor] = value_edit
    ...
    @staticmethod
    def _mt_label(text: str, width: int) -> QLabel:
        lbl = QLabel(text)
        lbl.setFixedWidth(width)
        lbl.setAlignment(Qt.AlignCenter)
        return lbl

    def _on_mt_dir_changed(self, motor: str, value: str):
        self._motor_test_dir[motor] = value

    def _on_mt_ctrl_changed(self, motor: str, value: str):
        self._motor_test_ctrl[motor] = value

    def _on_mt_value_changed(self, motor: str, text: str):
        try:
            self._motor_test_value[motor] = max(0, int(text))
        except ValueError:
            self._motor_test_value[motor] = 0

    def _motor_test_build_cmd(self, motor: str) -> str:
        """Build the H7 command string for one motor's current test config."""
        d = self._motor_test_dir[motor]
        ctrl = self._motor_test_ctrl[motor]
        val = self._motor_test_value[motor]
        if d == "Stop":
            return f"{motor} f0"
        if d == "Brake":
            return f"{motor} x"
        if d == "Forward":
            return f"{motor} fd{val}" if ctrl == "Duty" else f"{motor} f{val}"
        # Backward
        return f"{motor} bd{val}" if ctrl == "Duty" else f"{motor} b{val}"

    def _motor_test_send_commands(self):
        """Send configured test commands for all four motors."""
        for motor in self._MOTOR_TEST_MOTORS:
            self._send_cmd(self._motor_test_build_cmd(motor))

    def _motor_test_send_stop(self):
        """Send the global 'stop' command (motor test safety stop)."""
        self._send_cmd("stop")
    ...
    def keyPressEvent(self, event: QKeyEvent):
        ...
        if key_id in self.MOVEMENT_KEYS:
            ...
            if not self._motor_test_o_held:
                self._motor_test_o_held = True
                self._motor_test_send_commands()
        elif key_id in ("Q", "E"):

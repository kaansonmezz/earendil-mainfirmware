...
class EarendilControlGui(QMainWindow):
    ...
    def __init__(self):
        ...
        # Lightweight polling only fires while the serial port is connected
    ...
    #  All commands go through the existing _send_cmd() path.  The panel is
    ...
    def _send_drill_cmd(self, cmd: str):
        """Send a drill / mode command through the existing _send_cmd() path.
        ...
        if not self.connected:
            self._log_warn(f"[DRILL] Not sent (serial disconnected): {cmd}")
            return
        self._send_cmd(cmd)
    ...
    def _connect(self):
        ...
        try:
            ...
            # Arm / Drill polling no longer auto-starts on connect.
            # Use the "Start Arm Polling" / "Start Drill Polling" buttons
            # in the respective telemetry panels when needed.
        except Exception as e:
    ...
    def _on_rx_line(self, line: str):
        ...
        # Drill Telemetry parsers.  These run alongside the manipulation
        # parsers - the existing ``get motors`` request feeds both tables.
        # The drill mode / fault parsers are tolerant and return False for
        # lines that don't look like F401 mode/fault output.
        self._parse_drill_get_motors_line(line)
        self._parse_drill_mode_line(line)
        self._parse_drill_fault_line(line)
    ...
    def send_arm_setting_command(self, command: str):
        """Send one manipulation arm F401 setting command through _send_cmd().

        If the serial port is disconnected, _send_cmd() logs a warning and
        no exception is raised.  The command is also logged under the GUI
        console so the operator can see what would have been sent.
        """
        if not self.connected or not self.ser or not self.ser.is_open:
            self._log_warn(
                f"[ARM-SET] Not sent (serial disconnected): {command}")
            return
        self._log_info(f"[ARM-SET] {command}")
        self._send_cmd(command)

    def send_arm_setting_sequence(self, commands: list[str], interval_ms: int = 100):
        """Send a small sequence of arm setting commands through _send_cmd().
        ...
        if not self.connected or not self.ser or not self.ser.is_open:
            for cmd in commands:
                self._log_warn(
                    f"[ARM-SET] Not sent (serial disconnected): {cmd}")
            return

...
class MotorSettingsDialog(QDialog):
    ...
    def _apply_theme_style(self):
        """Style the dialog using the active main-window palette."""
        c = self._gui._colors()
        if self._gui.current_theme == "xp":
...
class EarendilControlGui(QMainWindow):
    ...
    XP_COLORS = {
        ...
        "border":             "#ACA899",
        "accent_gold":        "#003399",
        "accent_gold_bright": "#0055AA",
        "danger":             "#CC0000",
        "danger_bright":      "#E02020",
        "success":            "#008000",
        "success_bright":     "#00AA00",
        "warning":            "#CC8800",
        "selection_bg":       "#316AC5",
        "selection_border":   "#316AC5",
        "pressed_bg":         "#B8B5A8",
        "led_inactive_bg":    "#D4D0C8",
        "led_inactive_border":"#ACA899",
        "logo_opacity":       0.0,
        "manual_status_fg":   "#000000",
    ...
    THEMES = {"xp": XP_COLORS, "dark": DARK_COLORS, "light": LIGHT_COLORS}
    ...
    MOTOR_COL_HEADERS = [
        "Motor", "RPM", "Tgt", "Duty",
        "Dir", "State", "Mode", "Brake",
        "Fault", "Hall", "PWM Set", "PWM Act",
        "Drop", "RXB", "Err", "Link",
    ]
    ...
    def __init__(self):
        ...
        # ── State ──────────────────────────────────────────────────────────
        self.current_theme = "xp"              # "xp", "dark", or "light"
    ...
    def _build_rover_status_group(self) -> QGroupBox:
        ...
        self._btn_theme.setFixedWidth(75)
    ...
    def _build_mode_value_group(self) -> QGroupBox:
        ...
        self._lbl_value.setStyleSheet(
            "color: #0055AA; font-size: 12px; font-weight: bold;"
        )
    ...
    def _build_operating_mode_group(self) -> QGroupBox:
        ...
        self._lbl_op_mode_status.setFixedHeight(22)
        self._lbl_op_mode_status.setFixedWidth(280)
    ...
    def _build_motor_table_group(self) -> QGroupBox:
        ...
        self._motor_table.verticalHeader().setDefaultSectionSize(18)
        self._motor_table.horizontalHeader().setDefaultSectionSize(48)
        self._motor_table.setMaximumHeight(120)
        self._motor_table.setMinimumHeight(90)
    ...
    def _build_imu_group(self) -> QGroupBox:
        ...
        self._imu_table.verticalHeader().setDefaultSectionSize(16)
        self._imu_table.setMaximumHeight(65)
    ...
    def _build_console_group(self) -> QGroupBox:
        ...
        self._lbl_h7_console_title.setStyleSheet(
            "color: #003399; font-weight: bold; font-size: 9px;"
        )
        ...
        self._h7_console.setStyleSheet(
            "QTextEdit { background-color: #FFFFFF; border: 1px solid #7F9DB9; "
        ...
        self._btn_h7_send.setFixedWidth(50)
        ...
        btn_row = QHBoxLayout()
        btn_row.setSpacing(3)
        btn_clear_gui = QPushButton("Clear")
        btn_clear_gui.setFixedWidth(50)
        btn_clear_gui.clicked.connect(self._gui_console.clear)
        btn_row.addWidget(btn_clear_gui)

        btn_clear_h7 = QPushButton("Clear H7")
        btn_clear_h7.setFixedWidth(60)
        btn_clear_h7.clicked.connect(self._h7_console.clear)
        btn_row.addWidget(btn_clear_h7)
        btn_row.addStretch()
        gui_lay.addLayout(btn_row)
    ...
    def _toggle_theme(self):
        """Cycle between XP Compact, Dark, and Light themes.  No serial I/O."""
        cycle = ["xp", "dark", "light"]
        idx = cycle.index(self.current_theme) if self.current_theme in cycle else 0
        self.current_theme = cycle[(idx + 1) % len(cycle)]
        self._apply_theme()
        label_map = {"xp": "XP Compact", "dark": "Dark", "light": "Light"}
        next_map = {"xp": "Dark", "dark": "Light", "light": "XP Compact"}
        self._btn_theme.setText(f"{next_map[self.current_theme]}")
        self._log_info(f"Theme switched to {label_map[self.current_theme]}")
    ...
    def _style_badge(self, text_color: str) -> str:
        ...
        if self.current_theme == "xp":
            return (
                ...
                "border-radius: 0px; padding: 1px 6px; font-size: 9px;"
            )
        return (
            f"color: {text_color}; font-weight: bold; "
    ...
    def _style_console_widgets(self):
        ...
        if self.current_theme == "xp":
            self._h7_console.setStyleSheet(
                ...
                f"border: 1px solid #7F9DB9; "
                f"border-radius: 0px; color: {c['text']}; "
                "font-family: 'Consolas', 'Courier New', monospace; font-size: 10px; }}"
            )
            self._h7_input.setStyleSheet(
                f"QLineEdit {{ background-color: {c['bg_input']}; "
                f"border: 1px solid #7F9DB9; "
                f"border-radius: 0px; padding: 2px 4px; color: {c['text']}; "
                "font-family: 'Tahoma', 'MS Sans Serif', 'Sans Serif'; font-size: 9px; }}"
            )
            self._btn_h7_send.setStyleSheet(
                f"QPushButton {{ background-color: {c['bg_input']}; "
                f"border: 2px outset #D4D0C8; "
                f"border-radius: 0px; padding: 2px 8px; color: {c['text']}; "
                f"font-weight: bold; font-size: 9px; }}"
                f"QPushButton:hover {{ background-color: #E8E6DE; }}"
                f"QPushButton:pressed {{ border-style: inset; background-color: {c['pressed_bg']}; }}"
            )
            if hasattr(self, "_lbl_h7_console_title"):
                self._lbl_h7_console_title.setStyleSheet(
                    f"color: {c['accent_gold']}; font-weight: bold; font-size: 9px;"
                )
            if hasattr(self, "_lbl_gui_console_title"):
                self._lbl_gui_console_title.setStyleSheet(
                    f"color: {c['text_muted']}; font-weight: bold; font-size: 9px;"
                )
            self._gui_console.setStyleSheet(
                f"QTextEdit {{ background-color: {c['bg_console']}; "
                f"border: 1px solid #7F9DB9; "
                f"border-radius: 0px; color: {c['text_muted']}; "
                "font-family: 'Consolas', 'Courier New', monospace; font-size: 10px; }}"
            )
            return
    ...
    def _style_mode_value_labels(self):
        ...
        if self.current_theme == "xp":
            ...
            self._lbl_value.setStyleSheet(
                f"color: {value_color}; font-size: 12px; font-weight: bold;"
            )
            return
        if self.mode == "RPM":
            mode_color = c['accent_gold']
    ...
    def _style_operating_mode_status(self, cfg: dict):
        """Style the Operating Mode status box."""
        c = self._colors()
        if self.current_theme == "xp":
    ...
    def _make_led() -> QFrame:
        ...
        led.setStyleSheet(
            "QFrame { background-color: #D4D0C8; border: 1px solid #ACA899; "
            "border-radius: 6px; }"
        )
        return led
    ...
    def _show_help_popup(self):
        ...
        layout.setSpacing(8 if self.current_theme != "xp" else 4)

        title_font = "9px" if self.current_theme == "xp" else "18px"
        body_font = "9px" if self.current_theme == "xp" else "13px"
        title = QLabel("Earendil — Rover Control GUI Help")
        title.setStyleSheet(
            f"font-size: {title_font}; font-weight: bold; color: {c['accent_gold']};"
        )
        ...
        keys_html = (
            f"<table style='font-size:{body_font}; color:{c['text']};' cellspacing='8'>"
        ...
        mode_html = (
            f"<table style='font-size:{body_font}; color:{c['text']};' cellspacing='4'>"
